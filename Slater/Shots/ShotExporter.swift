...
enum ShotExporter {
    enum Format: String, CaseIterable, Identifiable {
        ...
        static var lastUsed: Format {
            get { UserDefaults.standard.string(forKey: "exportFormat").flatMap(Format.init) ?? .imageAndText }
            set { UserDefaults.standard.set(newValue.rawValue, forKey: "exportFormat") }
        }
    }

    /// Writes the files for `format` next to each other in `directory`, named after `baseName`:
    /// `<name>-original.png`, `<name>-translated.png` and `<name>.md`. If any would replace an
    /// existing file, all of them get a number (`<name> 2`) instead. Returns the files written.
    @discardableResult
    static func write(
        original: CGImage, translated: CGImage, markdown: String,
    ...
    private static func files(for format: Format, name: String, in directory: URL) -> [URL] {
        let images = ["\(name)-original.png", "\(name)-translated.png"]
        let text = ["\(name).md"]
        let names = switch format {
        case .imageAndText: images + text
        case .image: images
        case .text: text
        }
