...
For the same reason, Shots exist only in memory. Slater never writes Captures, Shots or translations to disk on its own: there is no history, no cache, and nothing is restored on relaunch. The only way anything is written to disk is the user explicitly saving a Shot, to a location they choose in a save dialog.
