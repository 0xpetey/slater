...
- **OCR accuracy, open issue:** across four Japanese fonts at 13pt (40 lines), the quick reading got 37 right and the corrected reading 39, and wherever they differed the corrected reading was right, so verification matters; on real screens the corrected reading changes about one Block per Shot, briefly visible as a swapped translation with an orange underline. Separately, *both* readings drop 当 (担当者, 当社, 該当) in Hiragino Sans W3 at 13pt, macOS's default Japanese font, so the disagreement signal can't catch that one. Needs its own investigation (fonts, sizes, Vision revisions).
