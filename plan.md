...
| App icon | A macOS squircle with a green gradient and the white lizard, generated at every size by `scripts/make-icon.swift` into `Slater/Assets.xcassets`. Apple's SF Symbols license allows the symbols in app UI but not in app icons or logos, so before Slater is distributed the icon's lizard must be replaced with an original drawing. | Asset catalog `AppIcon`, `ASSETCATALOG_COMPILER_APPICON_NAME` |
