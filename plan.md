...
- **Translation time:** macOS translates one text at a time, about 0.25–0.5 s each with the Accurate model, and batching, joining texts or parallel sessions don't help. Loading a model adds 1.5–8 s to the first call, so it's warmed up at launch, on wake and while you drag. A 20-cell spreadsheet takes several seconds, so results are streamed in reading order and each Block fills in as it's ready. The Fast model is the remaining lever; it's optional and downloaded from Settings.
...
- **OCR accuracy, open issue:** on a dense 13pt Hiragino W3 page, *both* readings dropped 当 from 担当者 and 両当事者, so the disagreement signal can't catch it. Needs its own investigation (fonts, sizes, Vision revisions).
