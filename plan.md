...
6. **Shot management and polish:** ShotStore, Open Shots menu section (thumbnail and the start of the translation), Close All, save to disk with ⌘S or the hover save button (`ShotExporter`; existing files are never replaced, a number is added instead), Settings (hotkey, launch at login, translation model), onboarding (permission, Fast model required, Accurate optional, launch at login on by default), the "No Japanese text found" notice, per-Shot Original / Fast / Accurate switch, and reading order that treats slightly misaligned table cells as one row.
7. **Later (optional):**
